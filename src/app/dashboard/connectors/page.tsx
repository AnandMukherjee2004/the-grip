...
export default function ConnectorsPage({ embedded = false }: { embedded?: boolean }) {
  ...
  const handleSyncNow = async () => {
    if (!viewingToolId) return;
    setSyncingToolId(viewingToolId);
    try {
      const workspaceId = localStorage.getItem("workspaceId") || localStorage.getItem("grip_workspace_id");
      const res = await fetch(`${API_URL}/api/v1/connectors?workspaceId=${workspaceId || ""}`);
      if (!res.ok) throw new Error("Failed to fetch connectors");
      const connectors = await res.json();
      const connector = connectors.find((c: any) => c.toolId === viewingToolId);
      ...
      if (!syncRes.ok) throw new Error("Failed to trigger sync");
    } catch (error) {
            ...
            <div className="space-y-6 animate-fadeIn">
              ...
              <div className="space-y-3">
                ...
                <div className="border border-gray-200 rounded-2xl bg-white shadow-sm p-6 space-y-6">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="space-y-1">
                      <h4 className="text-sm font-semibold text-gray-800">Sync now</h4>
                      <p className="text-xs text-gray-500">Manually trigger a data sync for this connector</p>
                    </div>
                    <button
                      type="button"
                      onClick={handleSyncNow}
                      disabled={syncingToolId === viewingToolId}
                      className="px-4 py-2 border border-gray-200 hover:bg-gray-50 rounded-xl text-xs font-semibold text-gray-700 shadow-sm flex items-center gap-1.5 transition-colors cursor-pointer disabled:opacity-50"
                    >
                      {syncingToolId === viewingToolId ? (
                        <>
