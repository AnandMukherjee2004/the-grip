...
export function AvailableToolCard({
  ...
  const handleModalSubmit = async (_apiKey: string, _apiSecret?: string) => {
  ...
  return (
    <>
        ...
        <div className="mt-3 pt-3 border-t border-white/5 flex items-center justify-end">
          {isConnected ? (
            ...
            <button
              ...
              className="h-7 px-3 rounded-lg bg-gradient-to-r from-purple-600 to-indigo-600 text-white font-bold text-[10px] hover:opacity-95 active:scale-[0.98] transition-all flex items-center justify-center gap-1.5 shadow-[0_0_12px_rgba(124,58,237,0.15)] cursor-pointer disabled:opacity-50"
            >
              {isConnecting ? (
                <>
      ...
      {/* API Key Modal if apikey auth method */}
      {isModalOpen && (
        <APIKeyModal
          tool={tool}
          config={config}
